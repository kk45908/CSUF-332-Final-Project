...
SELECT movie.title, movie_directors.first_name, movie_directors.last_name
FROM world.movie
JOIN movie_directors ON director_id = directorid;

SELECT City.Name, Country.Name AS CountryName
FROM world.City
JOIN Country ON City.CountryCode = Country.Code;

SELECT videogames.name, videogame_company.companyname
FROM world.videogames
JOIN videogame_company ON videogames.company = videogame_company.companyname;

SELECT music.name AS musictitle, music_artists.artistsname
FROM world.music
JOIN music_artists ON music.artist = music_artists.artistsname;

SELECT movie.title, movie_actors.first_name AS actors_firstname, movie_actors.last_name AS actors_lastname, movie_genres.genres_title
FROM movie
JOIN movie_genres ON movie.movie_id = movie_genres.movie_id
JOIN movie_actors ON movie.movie_id = movie_actors.movieid;
...
SELECT title
FROM world.movie
WHERE release_year = (SELECT release_year FROM world.movie WHERE title = 'A DEADLY Invitation');

SELECT companyname
FROM world.videogame_company
WHERE companyname IN (SELECT company FROM world.videogames GROUP BY company HAVING COUNT(*) > 1);

SELECT genre, COUNT(*) AS MusicCount
FROM world.music
GROUP BY genre
...
SELECT name, genre
FROM world.videogames
WHERE viedogame_release_country = (SELECT viedogame_release_country FROM world.videogames WHERE language = 'Japanese');

SELECT Name
FROM world.Country
JOIN world.movie ON Country.Code = movie.movie_release_country
GROUP BY Country.Code, Name;

SELECT 'Movie' AS ItemType, title, release_year
FROM world.movie
WHERE release_year = 2023

UNION

SELECT 'Music' AS ItemType, name, release_year AS ReleaseYear
FROM world.music
WHERE release_year = 2023

UNION

SELECT 'Video Game' AS ItemType, name, release_year
FROM world.videogames
WHERE release_year = 2023;

SELECT videogames.name, publisher.first_name, publisher.last_name
FROM world.videogames
JOIN world.videogame_company ON videogame_company.companyname = videogames.company
JOIN world.publisher ON publisher.publisher_id = videogame_company.publisher_id;
...
Having Count(Language) > 1 AND MAX(IsOfficial = 'T');

SELECT CountryCode, MAX(Language) AS Language, MAX(Percentage) AS MaxPercentage
